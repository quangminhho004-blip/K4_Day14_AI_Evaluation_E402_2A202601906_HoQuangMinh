...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an impartial evaluator. Score every rubric criterion from 0.0 "
            "to 1.0 and return only a JSON object mapping criterion names to scores.\n\n"
            f"Question:\n{question}\n\nAnswer:\n{answer}\n\nRubric:\n{rubric_text}"
        )
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures):
            ...
            suggestion = (
                ...
                else "Review the trace and add a targeted evaluation case"
            )
            rows.append(
    ...
    def generate_improvement_suggestions(
        ...
        if any(name.lower() == "hallucination" for name in categories):
            suggestions.append(
                "Add a grounding check that rejects claims unsupported by retrieved context"
            )
        if any(name.lower() == "irrelevant" for name in categories):
            suggestions.append(
                "Add intent-focused prompt examples so answers directly address the question"
            )
        if any("incomplete" in name.lower() for name in categories):
            suggestions.append(
                "Improve retrieval coverage and require answers to include policy conditions and exceptions"
            )
        if any(name.lower() == "off_topic" for name in categories):
            suggestions.append(
                "Add intent routing and an out-of-scope response policy before generation"
            )
        if any(name.lower() == "refusal" for name in categories):
            suggestions.append(
                "Calibrate guardrails with allowed support examples to reduce unnecessary refusals"
            )

        fallbacks = [
            "Review low-scoring traces and add representative cases to the regression dataset",
            "Tune chunking and retrieval ranking, then verify Context Recall and Context Precision",
            "Add response-format checks for required dates, amounts, conditions, and exceptions",
        ]
